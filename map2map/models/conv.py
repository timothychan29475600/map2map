...
class ConvBlockDropout(nn.Module):
    ...
    """
    def __init__(self, in_chan, out_chan=None, mid_chan=None,
            kernel_size=3, stride=1, seq='COBA',p=0.1):
        super().__init__()
        self.p = p
...
class ResBlockDropout(ConvBlockDropout):
    """Residual convolution blocks of the form specified by `seq`.
    Input, via a skip connection, is added to the residual followed by an
    optional activation.
    ...
    """
    def __init__(self, in_chan, out_chan=None, mid_chan=None,
                 kernel_size=3, stride=1, seq='COBACOBA', last_act=None,p=0.1):
        if last_act is None:
            last_act = seq[-1] == 'A'
        elif last_act and seq[-1] != 'A':
            warnings.warn(
        ...
        super().__init__(in_chan, out_chan=out_chan, mid_chan=mid_chan,
                         kernel_size=kernel_size, stride=stride, seq=seq,p=p)
