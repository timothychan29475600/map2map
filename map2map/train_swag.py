...
def swag_schedule(lr_init,lr_swag,epoch,swag_start):
  
    t = (epoch)/(swag_start)
    lr_ratio = lr_swag/lr_init
...
def gpu_worker(local_rank, node, args):
    ...
    if not args.swag:
        ...
        # SWAG Learning rate schedule
        last_epoch = -1
        high_lr_epoch = int(0.5*args.swag_start) # Epoch when lr start increasing/decreasing
        high_lr_val   = 1.0
        low_lr_epoch  = int(0.9*args.swag_start) # Epoch when lr stop increasing/decreasing
        low_lr_val    = args.swag_lr/args.lr


        scheduler1 = optim.lr_scheduler.ConstantLR(
            optimizer,
            factor = high_lr_val, 
            total_iters = high_lr_epoch,
            last_epoch = last_epoch,
            **args.scheduler_args)
        scheduler2 = optim.lr_scheduler.LinearLR(
            optimizer,
            start_factor = high_lr_val, 
            end_factor = low_lr_val, 
            total_iters = low_lr_epoch,
            last_epoch = last_epoch,
            **args.scheduler_args)
        scheduler = optim.lr_scheduler.SequentialLR(
            optimizer,
            schedulers=[scheduler1, scheduler2],
            milestones=[high_lr_epoch],
            last_epoch = -1,
            **args.scheduler_args)
    ...
    # Initialize SWA-Gaussian model (Maddox et. al. 2019) 
    if args.swag:
        # Flipping the flag just for convenience
        args.swag_no_cov_mat = not args.swag_use_conv
        
        SWAG = import_attr('SWAG', models, callback_at=args.callback_at)
        # Initialize model
        swag_model = SWAG(
            base = import_attr(args.model, models, callback_at=args.callback_at),
            no_cov_mat = args.swag_no_cov_mat,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Swag model averaging
        if args.swag and (epoch+1) > args.swag_start and (epoch+1 - args.swag_start) % args.swag_c_epochs == 0:
            # Model averaging
            swag_model.collect_model(model)

            #if epoch == 0 or epoch % args.swag_eval_freq == args.eval_freq -1 or epoch == args.epochs-1:
            #    pass # Evaluate model here (NOT IMPLEMENTED)
        ...
        if rank == 0:
            ...
            # Get separate files for swag
            logger.flush()
            state = {
                'epoch': epoch + 1,
                'swag_model': swag_model.state_dict(),
                'swag_loss': 0 #swag_loss, # (NOT IMPLEMENTED)
            }

            swag_state_file = 'swag_state_{}.pt'.format(epoch+1)
            torch.save(state,swag_state_file)
            del state

            tmp_link = '{}.pt'.format(time.time())
            os.symlink(swag_state_file,tmp_link)
            os.rename(tmp_link,'swag_checkpoint.pt')
